 ...
 *
 *  - We allow importing modules without an extension. This is a common practice
 *    in Node.js, but it is not allowed in ESM. We allow it here to maintain
 *    compatibility with a large number of modules.
 *  - We allow importing directories. This is not allowed in ESM, but it is
 *    common in Node.js. We allow it here to maintain compatibility with a large
 *    number of modules.
 ...
 *    as `default` conditions and packages without `export` conditions may still
 *    resolve to a CommonJS module.
 *
 * You can use `strict` to disable the above differences (which is recommended),
 * but you may need to fix some modules that are not compliant with the ESM
 * spec.
 *
